...
```css
/* dark theme file */
@media (prefers-color-scheme: dark) { html:not([data-mode]), [data-mode=""] {
  /* … */
}}
[data-mode="paint-it-black"] {
  /* … */
}
```
...
1. Create `src/design-tokens/tokens/themes/<theme>.json` with `_colorScheme`, `_name` metadata and `brand`, `base`, and `intent` groups.
...
Dark mode is supported through the `paint-it-black` theme which is loaded by default. Dark mode should be autmatically
activated based on the user's preference using `@media (prefers-color-scheme: {_colorScheme.value})`.
