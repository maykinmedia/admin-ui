...
located in `src/style/tokens`. These tokens must be compiled using [Style Dictionary](https://styledictionary.com/) before they can be used.
After compilation, the processed tokens are output to `src/style/compiled`, where they are consumed by the main
stylesheet in `src/style/index.scss`.
...
The token files in `src/style/tokens` are organized by role. During compilation:

- Tokens **without** a `type: "font"` attribute are merged and output to `src/style/compiled/properties.css`.
- Tokens **with** a `type: "font"` attribute are transformed into `@font-face` definitions and output to
`src/style/compiled/fonts.css`.
...
- **Test changes**: Verify styles in different themes and viewports to prevent regressions.
